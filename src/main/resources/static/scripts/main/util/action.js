(function (window, undefined) {
    ...
    function fUnFollowQuestion(oConf) {
        var that = this;
        that.post({
            url: '/zhihudemo/unfollowQuestion',
            data: {questionId: oConf.questionId},
            call: oConf.call,
            error: oConf.error,
            always: oConf.always
    ...
    function fPost(oConf) {
        ...
        $.ajax({
            ...
            if (oResult.code === 999) {
                // 未登录
                alert('未登录');
                window.location.href = '/zhihudemo/reglogin?next=' + window.encodeURI(window.location.href);
                return;
            }
